...
class MeetingForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'topic': forms.TextInput(attrs={'class': 'w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white', 'placeholder': 'Enter assignment name'}),
...
class EditMeetingForm(MeetingForm):
    recording_url = forms.URLField(
        required=False, 
        widget=forms.URLInput(attrs={'class': 'w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white', 'placeholder': 'https://... (Recording link)'})
    )

    class Meta(MeetingForm.Meta):
        fields = MeetingForm.Meta.fields + ['recording_url']
